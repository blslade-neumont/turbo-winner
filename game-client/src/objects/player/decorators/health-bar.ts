...
export class HealthBar extends GameObject {
    ...
    renderPlayerHealth(context: CanvasRenderingContext2D): void {
        const MAX_HB_WIDTH: number = 96 / this.scene!.camera!.zoomScale;
        const HB_OFFSET: number = -72 / this.scene!.camera!.zoomScale;
        const HB_HEIGHT: number = 16 / this.scene!.camera!.zoomScale;
        const HB_STROKE: number = 4 / this.scene!.camera!.zoomScale;
